...
const ShoppingCart = ({ isCartOpen, setIsCartOpen }) => {
  ...
  const handleCheckout = useCallback(async () => {
    if (cartItems.length === 0) {
      toast({
        title: t('messages.emptyCartCheckout'),
        description: t('messages.emptyCartDesc'),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t('messages.checkoutError'),
        description: t('messages.checkoutErrorDesc'),
        variant: 'destructive',
      });
    }
  }, [cartItems, clearCart, toast, t]);
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-2xl font-bold text-card-foreground">{t('cart.title')}</h2>
                ...
                cartItems.map(item => (
                  <div key={item.variant.id} className="flex items-center gap-4 bg-card border border-border p-3 rounded-lg">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <Button onClick={() => removeFromCart(item.variant.id)} size="sm" variant="ghost" className="text-destructive hover:text-destructive/90 text-xs">{t('cart.remove')}</Button>
                    </div>
                  </div>
                ))
              )}
