    ...
    if (product && selectedVariant) {
      ...
      try {
        ...
        toast({
          ...
          description: `${quantity} x ${product.title} (${selectedVariant.title}) ${t('messages.addedDesc')}`,
        });
      } catch (error) {
        toast({
          variant: "destructive",
    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center text-red-400 p-8 glass-card rounded-2xl">
          ...
          <p className="mb-6">{t('messages.errorLoadingProducts')} {error}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid md:grid-cols-2 gap-8 glass-card p-8 rounded-2xl">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            ...
            {product.variants.length > 1 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-white mb-2">{t('productDetail.style')}</h3>
